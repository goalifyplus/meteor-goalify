 ...
 *  id, reputation, created: from https://api.goalify.com/3/account/{username}
 *  email: from https://api.goalify.com/3/account/{username}/settings
 * Note that we *must* have an id. Also, this array is referenced in the
 * accounts-goalify package, so we should probably keep this name and structure.
 */
Goalify.whitelistedFields = ['id', 'email', 'reputation', 'created'];
...
const getTokens = function(config, query) {
	const endpoint = 'https://api.goalify.com/oauth2/token';
 ...
 *
 *  returns an object containing:
 *   id             {Integer}         The user's Goalify id
 ...
 *
 * @param   {Object} config       The OAuth configuration object
 * @param   {String} username     The Goalify username
...
const getAccount = function(config, username, accessToken) {
	const endpoint = `https://api.goalify.com/3/account/${username}`;
	let accountObject;
...
/**
 * getSettings gets the basic Goalify account/settings data
 *
 *  returns an object containing:
 ...
 *   pro_expiration          {Integer/Boolean}  False if not a pro user, their expiration date if they are.
 *   accepted_gallery_terms  {Boolean}          True if the user has accepted the terms of uploading to the Goalify gallery.
...
const getSettings = function(config, username, accessToken) {
	const endpoint = `https://api.goalify.com/3/account/${username}/settings`;
	let settingsObject;
	...
	try {
		...
		throw _.extend(new Error(`Failed to fetch settings data from Goalify. ${err.message}`), {
			response: err.response,
		});
	}
};
